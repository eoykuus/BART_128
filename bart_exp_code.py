...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\maste\\Desktop\\BART_Study\\bart_exp.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # Define event ID's as integers
    event_ids = {"start_balloon":1, "balloon_pump":2, "balloon_popped":3, "balloon_banked":4}
    ...
    for thisIntro_loop in intro_loop:
        ...
        countdownText = f"Deney {countdown} saniye içinde başlayacak."
    ...
    for thisTrial in trials:
        ...
        # Send LSL marker for new balloon
        outlet.push_sample([event_ids["start_balloon"]])
        ...
        for thisPump in pump:
            ...
            if popped:
                thisBalloonEarnings=0.0
                lastBalloonEarnings=0.0
                # Send LSL marker for balloon popped
                outlet.push_sample([event_ids["balloon_popped"]])
            else:
                lastBalloonEarnings=thisBalloonEarnings
                outlet.push_sample([event_ids["balloon_pump"]])
            
            if 'return' in bank_resp.keys:
                outlet.push_sample([event_ids["balloon_banked"]])
                pump.finished = True
